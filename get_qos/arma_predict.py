...
data=[]
records=QoS_data.objects.filter(ws_url="https://way.jd.com/jisuapi/weather?city=安顺&cityid=111&citycode=101260301&appkey=8604c9fc84720c85155a6a06770c2683")  # 各个web服务对应的最新QOS数据
...
def run_aram(df, maxar, maxma, test_size = 5):#  max_ar、max_ma是p、q值的最大备选值。
    data = df.dropna()
    ...
    model = sm.tsa.ARMA(x,(5,3)).fit(disp=0)
    # model.fit(disp=-1, method='css')
    print(len(train))
    print(len(data))
    test_predict=model.predict(len(train),len(data)-1)
