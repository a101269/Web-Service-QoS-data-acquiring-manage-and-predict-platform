    ...
    def __call__(self, value):
        if self.min_length and len(value) < self.min_length:
            raise forms.ValidationError(
                _('密码过短 (不少于 %s 字符)') % self.min_length,
                code=self.code)
        elif self.max_length and len(value) > self.max_length:
            raise forms.ValidationError(
                _('密码过长 (不大于 %s 字符)') % self.max_length,
                code=self.code)
...
class EmailDomainValidator(object):
    message = _('抱歉, %s 此邮箱无效. 请使用一个有效的邮箱地址.')
    code = 'invalid'
    ...
    def __call__(self, value):
        ...
        if not value or '@' not in value:
            raise forms.ValidationError(_('请输入一个有效邮箱地址.'), code=self.code)
